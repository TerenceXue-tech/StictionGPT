...
for i, filename in enumerate(os.listdir(input_dir)):
    if filename.endswith(".csv"):
        ...
        # 计算 |OP(k) - PV(k-1)|
        OP_shifted = np.array(OP[1:])  # OP(k)
        PV_shifted = np.array(PV[:-1])  # PV(k-1)
        ...
        plt.xticks([])
        plt.yticks([])
        plt.savefig(os.path.join(output_dir, f"{loop_name}{norm_suffix}{filter_suffix}_fly_p.svg"), format="svg")
        plt.clf()
        ...
        plt.xticks([])
        plt.yticks([])
        plt.savefig(os.path.join(output_dir, f"{loop_name}{norm_suffix}{filter_suffix}_fly_l.svg"), format="svg")
        plt.clf()
