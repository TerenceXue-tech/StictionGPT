...
def plot_loop_data(file_path, scene_name, loop_index, output_dir, normalize=False, apply_filter=True):
    ...
    try:
        ...
        # 提取 PV, OP, t, SP 数据
        PV_raw = np.squeeze(loop_data[0][0][0][0][6])
        OP_raw = np.squeeze(loop_data[0][0][0][0][7])
        t = np.squeeze(loop_data[0][0][0][0][4])
        SP = np.squeeze(loop_data[0][0][0][0][5])
        ...
        plt.xticks([])
        plt.yticks([])
        plt.savefig(os.path.join(output_dir, f"{loop_name}{i}{norm_suffix}{filter_suffix}_fly_p.svg"), format="svg")
        plt.clf()
        ...
        plt.xticks([])
        plt.yticks([])
        plt.savefig(os.path.join(output_dir, f"{loop_name}{i}{norm_suffix}{filter_suffix}_fly_l.svg"), format="svg")
        plt.clf()
...
scene_name = "buildings"

for i in range(1, 9):
    loop_index = f"loop{i}"
    output_dir = f"./fig/BAS{i}"
    plot_loop_data(file_path, scene_name, loop_index, output_dir, normalize=True, apply_filter=0)
